...
def gameList(request):
    ...
    games_on_sale = Game.objects.filter(promotion__isnull=False).order_by("-promotion")[
        :4
    ]
...
def gameView(request, id):
    ...
    has_purchased = PurchaseItem.objects.filter(
        purchase__user=request.user, game=game, refunded=False
    ).exists()
...
def cartView(request):
    ...
    context = {"cart_items": user_cart, "total": total}
...
def addcartItemView(request, game_id):
    ...
    try:
        ...
        return HttpResponseBadRequest(
            '<script>alert("Este jogo já está no carrinho."); window.history.back();</script>'
        )
...
def updatecartitemView(request):
    ...
    try:
        ...
        return HttpResponseBadRequest(
            '<script>alert("Este jogo não está no carrinho."); window.history.back();</script>'
        )
...
def purchaseView(request):
    ...
    if cart_items:
        ...
        for cart_item in cart_items:
            ...
            PurchaseItem.objects.create(game=game, purchase=purchase, price=price)
...
def refundGameView(request, game_id):
    ...
    purchase_items = PurchaseItem.objects.filter(game=game, purchase__user=request.user)
